...
class EntradasusuarioExport implements FromQuery,withHeadings
{
    use Exportable;
    ...
    public function query()
    {
        ...
        return Entrada::query()->join('productos','entradas.producto_id','productos.id')->join('usuarios','entradas.usuario_id','usuarios.id')->join('proveedores','entradas.proveedore_id','proveedores.id')->join('categorias','productos.categoria_id','categorias.id')->where('entradas.tienda_id','=',$this->tienda_id)->where('usuarios.id','=',$this->id)->whereBetween('entradas.fechaentrada',array($this->inicio,$this->final))->orderBy("entradas.created_at", "desc")->select("entradas.fechaentrada","entradas.precio","entradas.stock","entradas.precioentrada","entradas.stockentrada","entradas.precioventa","entradas.precioentradaven","entradas.descripcion","categorias.categoria","productos.codigo","productos.nombre","usuarios.usuario","proveedores.nombre as nombrepro");
